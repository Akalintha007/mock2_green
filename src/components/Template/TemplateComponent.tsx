...
const TemplateComponent = () => {
    ...
    useEffect(() => {
        const processor = async () => {
            let response = await fetch('http://localhost:8000/extra');
            let data1 = await response.json();
            setData(data1); 
            response = await fetch('http://localhost:8000/library');
            ...
            <Router>
                ...
                <Box sx={{position: 'absolute', top: '93px', width: '100%'}}>
                      <Routes>
                        ...
                        <Route path="book-info/:bookId1" element={<BookInfo library={library} setLibrary={setLibrary}/>} />
